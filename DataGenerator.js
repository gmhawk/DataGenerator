...
var maxChucks = 20; // Change to set Chunks
var chunkSize = 500000; // Change to set ChunkSize
// Retain and Release
function retainer(currentLoop) {
  ...
  if (currentLoop <= maxChucks) {
    ...
    for (var i = 0; i < chunkSize; i++) {
      ...
      let imageURL = faker.image.business();
    ...
    fs.appendFile('./DO_NOT_OPEN/sdc_data.json', collectedWater, (err) => {
      if (err) {console.error(err)}
      console.log(`Currently at loop #${currentLoop}`);
      looped++;
      retainer(looped);
    });
  }
